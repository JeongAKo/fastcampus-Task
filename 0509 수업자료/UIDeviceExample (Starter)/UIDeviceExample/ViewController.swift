...
final class ViewController: UIViewController {
    ...
    @IBAction private func systemVersion() {
        ...
        // 12.2.3 // Major, Minor(public), Minor(nin-public)
        let splitVer = systemVersion.split(separator: ".").compactMap {Int($0)}
        print(splitVer)
    ...
    @IBAction private func deviceModel() {
        ...
        print("isMultitaskingSupported :", device.isMultitaskingSupported) //최신기기에서는 항상 true
    ...
    @IBAction private func batteryMonitoring(_ sender: UIButton) {
        ...
        if device.isBatteryMonitoringEnabled {
            notiCenter.addObserver(self,
                                   selector: #selector(didChangeBatteryState(_:)), name: UIDevice.batteryStateDidChangeNotification, object: nil
            )
        } else {
    ...
    @IBAction private func proximityMonitoring(_ sender: UIButton) {
        ...
        print("ProximityMonitoering: ", device.isProximityMonitoringEnabled)
        
        if device.isProximityMonitoringEnabled {
            notiCenter.addObserver(self,
                                   selector: #selector(didChangeProximityState(_:)), name: UIDevice.proximityStateDidChangeNotification, object: nil
            )
        } else {
            notiCenter.removeObserver(self,
    ...
    @objc func orientationDidChange(_ noti: Notification) {
        ...
        if let device = noti.object as? UIDevice {
            print("Sevice Orientation :", device.orientation)
        }
        print("statusBarOrientation :", UIApplication.shared.statusBarOrientation)
    }
