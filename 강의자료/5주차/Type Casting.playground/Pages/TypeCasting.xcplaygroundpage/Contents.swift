...
type(of: rect as Shape) is Rectangle
...
//반드시 성공하는 타입변환 - 부모클래스거나 자기자신
//shape as Rectangle - 부모클래스 -> 자식클래스 변환
/*:
 ---
 ## Downcasting
 ---
...
//let downcastedRect: Rectangle = shapeRect as Rectangle   //as 는 항상 성공하는 애만 써야한다

//as? 타입변환이 성골하면 Optional 값으로 들어가고 실패하면 nill
//as! 성공하면 값이 그대로 드어가고 실패하면 에러
...
let value = 1
//(value as Float) is Float   // Int 와 Float 는 관계가 없다 / 컴파일 에러
//(value as? Float) is Float  // nil을 반환해서 nil is float 와 같다 / false
//(value as! Float) is Float  // 런타임 에러
 ...
 */
//for elem in list {
//  if elem is Shape { //자식클래스 is 부모클래스 항상 true
