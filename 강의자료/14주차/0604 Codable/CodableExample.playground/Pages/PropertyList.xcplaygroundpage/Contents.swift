...
let macBook = MacBook(
  model: "MacBook Pro", modelYear: 2015, display: 15
)
 ...
 */
    print("--prac encoding--")
    let pracArr = [macBook, macBook, macBook]
    let pracPListEncoder = PropertyListEncoder()

    let prackEncodedMacBook = try! pracPListEncoder.encode([pracArr])

    print(prackEncodedMacBook)

    print("--prac decoding--")
    let pracPListDecoder = PropertyListDecoder()
//    if let pracDecodedMacbook = try? pracPListDecoder.decode([pracArr], from: prackEncodedMacBook) {
//    print(pracDecodedMacbook)
//}
